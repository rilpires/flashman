        ...
        .addClass('col-lg m-1 grey lighten-4').append(
          $('<div>').addClass('row pt-2').append(
            ...
            $('<button>').addClass('btn btn-primary btn-sm my-0 col')
                         ...
                         .attr('type', 'button')
                         .prop('disabled',
                               isBridge || !(isSuperuser ||
                               grantLanDevicesBlock))
            .append(
              (device.is_blocked) ?
          ...
          $('<div>').addClass('row pt-3 mb-2').append(
            ...
            $('<div>').addClass('col').append(
              $('<h6>').text(
                ((device.wifi_freq) ? device.wifi_freq : 'N/D') + ' GHz'),
              $('<h6>').text(
                'Modo: ' + ((device.wifi_mode) ? device.wifi_mode : 'N/D')),
              $('<h6>').text(
                'Sinal: ' +
                ((device.wifi_signal) ? device.wifi_signal : 'N/D') +' dBm'),
              $('<h6>').text(
                'SNR: ' + ((device.wifi_snr) ? device.wifi_snr : 'N/D') + ' dB')
              .append(
                $('<span>').html('&nbsp'),
                ((device.wifi_snr >= 25) ?
                 $('<i>').addClass('fas fa-circle green-text') :
      ...
      $.each(lanRouters[routerMacKey], function(idx, router) {
        lanRouterCard.append(
          ...
          $('<div>').addClass('row pt-2 m-0 mt-1 grey lighten-3').append(
            ...
            $('<div>').addClass('col').append(
              $('<h6>').text(
                'Velocidade de recepção: ' + router.rx_bit + ' Mbps'),
