...
flashifyModel.portForwardPermissions = {
  noAsymNoRanges: {
...
flashifyModel.identifier = {vendor: 'NoVendor', model: 'NoName'};
...
flashifyModel.convertWifiMode = function(mode) {
  switch (mode) {
    case '11g':
      return '11bg';
    case '11n':
...
flashifyModel.convertField = function(
...
flashifyModel.convertIGDtoDevice = function(fields) {
  Object.keys(fields).forEach((k) => {
    if (typeof fields[k] === 'object' && !Array.isArray(fields[k])) {
      return flashifyModel.convertIGDtoDevice(fields[k]);
...
flashifyModel.isAllowedWebadminUsername = function(name) {
  return true;
};
...
flashifyModel.convertSpeedValueBasic = function(endTime, beginTime, bytesRec) {
...
flashifyModel.allowedFirmwareUpgrades = function(fwVersion, permissions) {
  if (
...
flashifyModel.assocFieldWildcardReplacer = function(assocFieldKey, ifaceIndex) {
...
flashifyModel.assocDevicesWildcardReplacer = function(assocDevicesKey,
...
// very basic changes. "Base" is exactly what is exported below: flashifyModel.
